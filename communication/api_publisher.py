...
Endpoint tunggal: EFWS_API_URL/sensors/telemetry  (settings.telemetry_endpoint())
"""
import json
import time
import logging
...
class APIPublisher:
    def __init__(self):
        ...
        self._connectivity_check_interval = settings.EFWS_CONNECTIVITY_CHECK_SEC  # default 120s (2 menit)

    # ─── Connectivity ─────────────────────────────────────────────
    def _is_reachable(self) -> bool:
        try:
            self.session.get(settings.API_BASE_URL, timeout=5,
    ...
    def _post(self, endpoint: str, payload: dict) -> bool:
        ...
        for attempt in range(1, settings.API_MAX_RETRIES + 1):
            if self._post_once(endpoint, body):
                ...
                self.online = True
                logger.info("✅ Terkirim ke %s (ts=%s)",
                            endpoint.split("/")[-1], payload.get("telemetry", [{}])[0].get("timestamp", "")[:19])
                return True
    ...
    def send_telemetry(self, payload: dict, db=None) -> bool:
        """
        Kirim payload telemetry ke EFWS_API_URL/sensors/telemetry.
        Endpoint dibaca dinamis dari env setiap kali dipanggil.
        Jika gagal DAN db diberikan → masuk offline queue.
        """
        ...
        if not success and db is not None:
            ...
            logger.warning("📦 Telemetry disimpan ke offline queue (sinyal tidak ada).")
        return success
    ...
    def flush_queue(self, db, batch_size: int = 10) -> int:
        ...
        for item in pending:
            ...
            # Remap endpoint ke URL terkini dari env — kalau EFWS_API_URL diubah,
            # queue lama otomatis ikut URL baru
            endpoint = settings.telemetry_endpoint()
            if endpoint != item["endpoint"]:
                logger.info("  🔀 Queue #%d: endpoint remapped → %s", item["id"], endpoint)

            if self._post_once(endpoint, json.dumps(payload, default=str)):
                db.mark_queue_sent(item["id"])
                sent += 1
                logger.info("  ✅ Queue #%d terkirim", item["id"])
            else:
                db.mark_queue_failed(item["id"], "flush interrupted")
                self.online = False
                self._last_connectivity_check = time.time()
                logger.warning("  ⏸ Flush berhenti di #%d — koneksi putus.", item["id"])
                break

        if sent:
            logger.info("📤 Flush selesai: %d terkirim, %d pending.",
                        sent, db.count_pending_queue())
        return sent
